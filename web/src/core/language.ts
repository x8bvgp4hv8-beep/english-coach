...
export const LANGUAGES: readonly LearningLanguage[] = [
  {
    ...
    adverb: 'по-английски',
    greeting: 'Hello!',
    // Строка про объём обязана меняться вместе с контентом: она уже один раз соврала —
    // «пока по полтора часа каждый» осталось после того, как A1–B2 наполнили до 90–176
    // часов. Размер уровня приложение считает само (`ProgressionEngine.hours`), здесь
    // только то, что от наполнения не зависит.
    note: 'Пять уровней от A1 до C1, размер каждого — под выбором',
    speechLocale: 'en-GB',
  ...
  },
  {
    ...
    adverb: 'по-испански',
    greeting: '¡Hola!',
    // A2 наполнили после того, как эта строка была написана: теперь это 30 блоков и
    // 510 уроков, столько же, сколько A1. Строка обещала «час» — то есть врала вдвое
    // больше, чем на девяносто часов занятий.
    note: 'A1 и A2 — по 90 часов с нуля; дальше B1–C1, пока по часу',
    speechLocale: 'es-ES',
