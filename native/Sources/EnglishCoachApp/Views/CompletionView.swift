...
struct CompletionView: View {
    ...
    var body: some View {
        VStack(spacing: 22) {
            ...
            HStack(spacing: 30) { metric("sparkles", "\(model.totalPoints)", "очков"); metric("flame.fill", "\(model.streak())", "дней подряд") }.padding(20).background(.white.opacity(0.8), in: RoundedRectangle(cornerRadius: 20))
